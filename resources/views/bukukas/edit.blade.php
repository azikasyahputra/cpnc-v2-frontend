...
<div class="col-md-12 col-sm-12 col-12">
    <form id="demo-form" class="form-horizontal" action="{{URL::route('kassaveedit')}}" method="post">
        @foreach($header as $dataheader)
        {{ csrf_field() }}
        <input type="hidden" value="{{$dataheader->id_kas}}" name="id_kas">
    <div class="card">
        <div class="card-body">
                <div class="row">
                ...
                <div class="row">
                    <table class="table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <input type="hidden" value="{{$datadetail->id_kas_dt}}" name="id_kas_dt_{{$i}}">
                                <td style="width:300px">
                                    <select id="referensi_{{$i}}" name="referensi_{{$i}}" class="selectkas form-control">
                                        <option value="kosong">-</option>
                                         @foreach($referensi as $referensi2)
                                            <option value="{{e($referensi2->id_referensi)}}" @if(e($referensi2->id_referensi)==$datadetail->id_referensi) selected @endif>{{e($referensi2->kode_referensi)}} - {{$referensi2->keterangan_referensi}}</option>
                                         @endforeach
                                    </select>
                                </td>
                                <td style="width:300px">
                                    <input type="text" id="keterangan_{{$i}}" class="form-control" name="keterangan_{{$i}}" value="{{$datadetail->keterangan}}" />
                                </td>
                                <td>
                                    <input type="hidden" id="debit_{{$i}}" class="cost" name="debit_{{$i}}" value="{{$datadetail->biaya_debit}}"/>
                                    <input type="text" id="show_debit_{{$i}}" class="form-control text-end" name="show_debit_{{$i}}" value="<?php if($datadetail->biaya_debit != 0){echo 'Rp. '.number_format($datadetail->biaya_debit,0,'','.');}else{echo $datadetail->biaya_debit;}?>" onkeyup="formatRupiah(event)"/>
                                </td>
                                <td>
                                    <input type="hidden" id="kredit_{{$i}}" class="cost2" name="kredit_{{$i}}" value="{{$datadetail->biaya_kredit}}"/>
                                    <input type="text" id="show_kredit_{{$i}}" class="form-control text-end" name="show_kredit_{{$i}}" value="<?php if($datadetail->biaya_kredit != 0){echo 'Rp. '.number_format($datadetail->biaya_kredit,0,'','.');}else{echo $datadetail->biaya_kredit;}?>" onkeyup="formatRupiah(event)"/>
                                </td>
                            </tr>
                            <?php $i++;?>
                            @endforeach
                            ...
                            @while ($n<7)
                            <tr>
                                <input type="hidden" value="" name="id_kas_dt_{{$n}}">
                                <td style="width:300px">
                                    <select id="referensi_{{$n}}" name="referensi_{{$n}}" class="selectkas form-control">
                                ...
                                <td style="width:300px">
                                    <input type="text" id="keterangan_{{$n}}" class="form-control" name="keterangan_{{$n}}"/>
                                </td>
                                <td>
                                    <input type="text" id="debit_{{$n}}" class="cost form-control text-end" name="debit_{{$n}}"/>
                                </td>
                                <td>
                                    <input type="text" id="kredit_{{$n}}" class="cost2 form-control text-end" name="kredit_{{$n}}"/>
                                </td>
                            </tr>
                            <?php $n++;?>
                            @endwhile
                ...
                <div class="row">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-10 col-xs-10 mb-3 ">
                           <input type="hidden" id="sum" name="total_debit" value="{{$dataheader->total_debit}}" required />
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-10 col-xs-10 mb-3 ">
                           <input type="hidden" id="sum2" name="total_kredit" value="{{$dataheader->total_kredit}}" required />
